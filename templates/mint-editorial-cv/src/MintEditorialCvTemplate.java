 ...
 *       schema is mirrored in {@link MintEditorialCvSpec}; see the
 *       record's Javadoc for field-by-field details.</li>
 ...
 *       {@code assets/asset-request.json}. Resolve a new icon by
 *       editing the request, re-running the asset-resolver, then
 *       updating {@link #ICONS} to point at the new file.</li>
 * </ul>
 *
 * <h3>Spaced-uppercase styling rule</h3>
 *
 * Every section heading and label in the reference uses spaced
 * uppercase ("R O S E  H A R R I S", "C O N T A C T",
 * "A W A R D S"). The template applies this transformation at render
 * time via {@link #letterSpace(String)}; the JSON carries natural-case
 * strings ("Rose Harris", "Contact", "Awards"). When you want a
 * particular field to NOT be letter-spaced, render it via
 * {@code body(...)} or directly with a {@link DocumentTextStyle}
 * instead of via {@link #heading} / {@link #label}.
 *
 * <h3>Clickable links</h3>
 *
 * <ul>
 *   <li>{@code contact[].url} wraps the contact row in a
 *       {@link DocumentLinkOptions}; use {@code mailto:}, {@code tel:},
 *       or {@code https://} as the scheme.</li>
