...
## Phase 6 note: PDFs and previews

This tool does **not** render `output.pdf` or `output.png`. Every new or copied revision marks those files as `pendingArtifacts`. Pair `graphcompose-flow` with the Phase 6 preview-renderer (planned) to produce the rendered artifacts after each revision change.
