 ...
 * nastier of the two, because it loads and runs. This CLI rejected flags it
 * predated — `error: unknown option '--report'`, exit 1 — and, worse, exited 0
 * from a `new-revision` that carried no sources forward because it was compiled
 * before carrying them existed. Because dist/ is gitignored, `git stash` does
 * not reach it, so a stale build survives every attempt to revert it away: two
 * red `pass.mjs --open` tests reproduced on what looked like a pristine tree.
 *
...
export function requireBuild(name, entryFile) {
  if (!fs.existsSync(path.join(distDir, entryFile))) {
    process.stderr.write(
      `${name} is not built yet (tools/revision-manager/dist is missing).\n` +
  ...
  if (fs.existsSync(srcDir) && newestMtimeMs(srcDir) > newestMtimeMs(distDir)) {
    process.stderr.write(
      `${name} is built from a stale dist/ (tools/revision-manager/src has changed since\n` +
        "it was compiled). Running it would reject flags it predates and silently\n" +
        "skip work it does not know about, so it stops here instead.\n\n" +
        "Rebuild from the repository root:\n\n" +
        "    npm run build --prefix tools/revision-manager   # or, for everything: npm run setup\n\n",
    );
    process.exit(69);
  }
}
