...
**The version under test is 0.7.1.** What it has that v1's run did not,
in the order the run will meet them:

| | v1 (0.5.0-beta.9 era) | 0.7.1 |
|---|---|---|
...
6. Record the harness version (`package.json` — 0.7.1 at the time of
...
```text
                 v1 create      0.7.1 create     delta
wall clock       68 min         ?                ?
requests         211            ?                ?
output tokens    280.4k         ?                ?
cache read       61.0M          ?                ?

                 v1 correction  0.7.1 correction delta
wall clock       7-10 min       ?                ?
requests         32-39          ?                ?
```
