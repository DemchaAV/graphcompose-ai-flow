...
| One loop pass: render + diff + verdict | `node scripts/render-and-diff.mjs --project <id> --revision <id>` — exit 0 ready, 2 revise, **4 the loop spent its budget with work open**, 3 blocked (no usable document). Every run is appended to the revision's `attempts.json` (figure, worst regions, causes, a fingerprint of the sources), so a sweep is counted as the renders it is |
