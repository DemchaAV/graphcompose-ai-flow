...
| **Commit a discovery artifact** (never write the canonical path) | `node scripts/write-artifact.mjs --project <id> --artifact <name> --from <draft\|->` — stages beside the target, validates it against its schema, renames. The canonical path holds the previous complete artifact or the new one, never half of either; a rejected draft leaves it untouched and names the schema it failed. Exit 0 written, 1 rejected. The `Write` guard refuses the direct write |
| **What each discovery worker reads, owns and is finished by** | `node scripts/check-analysis.mjs --contract geometry\|content\|assets` — the input contract declared in `config/pipeline.json`, printed for pasting into a worker's prompt, so three prompts quote one declaration instead of carrying three copies |
| **Close discovery: record the boundary** | `node scripts/handoff.mjs write --project <id>` — runs the authoring barrier and, only if it is clear, records `handoff.json`: the five artifact paths, their sha256s, and that the barrier passed. A red barrier writes nothing. `show` prints it; `verify` re-hashes and exits 1 when an artifact has changed or gone. Paths and hashes only, never contents |
...
| **One loop pass, as two commands and one screen** | `node scripts/pass.mjs --project <id> --open "<what this pass fixes>" [--report "<user's words>"]` opens the next revision (sources carried forward) and prints what the pass is aimed at — focus, the evidence's owner and properties, what has been tried, the budget. Edit one owning property. Then `node scripts/pass.mjs --project <id>` renders, measures, runs every gate and prints one screen: the figure and its movement, the worst regions with cause and owner, the checks, the loop line, the next step. Then write the review and ask `iterate-status`. Three calls per pass |
| Open a revision | `node tools/revision-manager/bin/graphcompose-flow.mjs new-revision "<gesture>" --project <dir>` — carries the parent's sources forward (template, data, assets, analysis, plan), not its render or review |
| **The user said something looks wrong** | `… new-revision "<gesture>" --report "<their words verbatim>" --project <dir>` — writes `human-report.json`; `iterate-status` keeps it in front of every measured mismatch until a review sets `humanReportedMismatch.addressed: true`, and the pass it opened is not charged to the budget |
| One loop pass: render + diff + verdict | `node scripts/render-and-diff.mjs --project <id> --revision <id>` — exit 0 ready, 2 revise, **4 the loop spent its budget with work open**, 3 blocked (no usable document). Every run is appended to the revision's `attempts.json` (figure, worst regions, causes, a fingerprint of the sources), so a sweep is counted as the renders it is. Beside the pixel percentage it reports `perceptual` (SSIM over the downsampled luminance — 0.44 to 0.95 over the audited corpus where the pixel figure sat at 5–12% on every revision) and builds the evidence for the regions carrying the difference by mass, each with a **measured** shift: the region's ink on both rasters subtracted, or the reference crop correlated over the render |
| **Something cannot be fixed on this line** — a typeface no bundled family reproduces, an API the version lacks | `node scripts/limitations.mjs accept <id> --project <id> --reason "<what was measured and why it is acceptable>" --mismatch <review-mismatch-id>` — recorded once; from then on it is never the focus, never counts toward the same-cause bound, and never blocks READY. `list` · `covers --mismatch <id>` · `retire <id> --note "<what changed>"` |
| Render only | `node scripts/render.mjs <project-id> <revision-id> [--root <workspace>]` — the runner compiles through `javac` with a cached classpath (≈6 s a pass on a warm machine, from 15); the debug render with guide lines runs only with `RENDER_DEBUG=1` / `pass --debug`; one render per project at a time (`.render.lock` beside `template-project.json` names the holder; a dead holder's lock is taken over) |
...
| **What did this pass replace?** | `node scripts/source.mjs diff --project <id> --revision <id>` — the share of methods this revision changed against its parent. Under 20% is a correction; most of them is a different construction, which belongs in a revision of its own. `render-and-diff` runs it every pass |
| **Read one method of the template** | `node scripts/source.mjs outline --project <id> --revision <id>` — every method, its line range and its size, for about a fortieth of the file. Then `symbol <name>` for the one you want, Javadoc included. **Do not `sed` or `cat` a template**: measured over one run, `sed` and `cat` returned 48k tokens across 35 calls — more than twice everything the nine deterministic tools returned across ninety — and all of it was hunting for one method. `constants` lists what a correction actually edits |
...
| **What kind of thing is wrong?** | Already answered: every `render-and-diff` pass writes `evidence.json` for the three worst regions and puts their causes on its `evidence` line. The cause restricts the fix — a geometry edit against a typography cause moves the wrong picture into place. `node scripts/evidence.mjs --project <id> --revision <id> --region <id>` (`--mismatch <id>`, `--all`, `--worst <n>`) — joins the reference regions, the measured pixel difference and the layout snapshot into ~4 KB: the owning node, its displacement, and the properties that produced its position. Assigns only what two measurements settle; `UNKNOWN` with candidates otherwise |
...
| **What does the reference measure?** | `node scripts/reference.mjs analyze --project <id> --json` — page, margins, palette, rules, columns and their gutters, and the text bands **per column**, in one call and about 5 KB. This is the first measuring call, not the thirteenth: the questions it answers need no window and no judgement, and a run that asked them one at a time reached authoring after ninety calls. `measure` / `rules` / `bands` / `colors` / `compare` are for the questions after it, where choosing a window IS the judgement |
...
| **Is this a layout, or a calibration of one reference?** | `node scripts/check-calibration.mjs --project <id> --revision <id>` — reads the template for a reference-pixel scale, `px(…)` arithmetic in render methods, a face's metrics as layout terms, and two-decimal literals nobody derived; exit 2 means the approval gate would refuse it. A theme may carry calibrated tokens; a render method may not |
| Approve and publish | `node scripts/approve-and-publish.mjs --project <id>` — one command: the quality gate (structural smells + calibration), approve, publish, README, verify, telemetry `finish`. `--waive-quality "<reason ≥ 60 chars>"` approves over the gate and records why in `quality-waiver.json` |
| **An engine defect on record → an issue in the library** | `node scripts/observations.mjs issue <id> [--repo owner/name]` — writes the GitHub issue from the observation (behaviour, API, reproduction, measurements, workaround) into the store's `issues/` and prints the `gh issue create` command; refuses one already reported or fixed. Record the URL in `engineDefect.reportedTo` afterwards |
...
| **Which part of a run cost the money** | `node scripts/telemetry/run-metrics.mjs phases --project <id>` — per-phase requests, tokens, tool calls, context growth and wall clock, with the discovery workers attributed by name; plus artifact sizes, render count and time to first render. `report` says what a run cost; only this says which part of it, and on two recorded runs the answer was the loop (84%, 96%) rather than the fan-out (3.0%, 0.0%). Where a host reports no usage it omits the table rather than printing zeros |
