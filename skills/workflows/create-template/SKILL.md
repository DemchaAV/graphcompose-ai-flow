...
| **2. Analyse** — measure the reference in one call, describe it as regions with roles, flow and data, resolve the assets it names, and record the handoff | `visual-analysis.json`, `<doc-kind>-data.json` (+ overflow dataset when it flows), `asset-request.json`, `assets-manifest.json`, `architecture-plan.json`, `handoff.json` | [create-2-analyse.md](../references/create-2-analyse.md) |
| **3. Author** — start from the handoff, check the plan and manifest are complete (`check-analysis --for authoring`), then write the template against the pinned pack's allow-list | the template class, the spec provider | [create-3-author.md](../references/create-3-author.md) |
...
Phase 2 and phase 3 are separated by a **context boundary**: when the
handoff is written, everything authoring needs is on disk, and the
discovery conversation is no longer an input. Phases 3 and 4 are not
separated — the render loop's passes are corrections to each other and
keep one context.
