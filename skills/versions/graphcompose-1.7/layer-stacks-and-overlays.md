...
## Per-layer offsets with position(...) (1.7.0 fix)

A badge or cap placed with `position(node, dx, dy, align)` is anchored
by `align` and then nudged by the `(dx, dy)` offset. Before 1.7.0 those
offsets were silently dropped when the `LayerStack` / `ShapeContainer`
was nested inside a fixed slot — a row column or another layer — so a
positioned badge could not be moved off its anchor once nested, even
though the same call worked at the document root. 1.7.0 fixes this: the
nested path now feeds the same offsets as the root path, so
`position(badge, -6, 5, LayerAlign.TOP_RIGHT)` nudges identically
whether the stack is at the root or nested in a row cell.

Implications for this workflow:

- Target 1.7.0 (this pack's version) before relying on a nudged badge
  inside a nested stack. On 1.6.x the offset is ignored and the badge
  sits exactly on its anchor — if a template must run on 1.6.x, anchor
  the badge with `align` alone and accept the on-anchor position.
- Layouts that did not use `position(...)` inside a nested stack are
  unchanged; this is a fix, not a layout shift.
- Prefer `align` + a small `(dx, dy)` offset over a hand-computed
  absolute position, per the anchors-over-offsets principle.
