...
2. The hosted Javadoc at the version pinned in the skill manifest's
   `verifiedAgainst` (currently 1.7.0).
...
exact method overloads against the 1.7.0 Javadoc before relying on a
specific signature, per the lookup priority above.

## Known limitations

This skill describes the 1.7.x semantics conceptually. It does not
list the exact method signatures of every builder. Specific method
signatures must be cross-checked against the
[`verifiedAgainst: 1.7.0`](../../../docs/versioned-skills.md) examples
