...
4. The hosted Javadoc at the version pinned in the skill manifest's
   `verifiedAgainst` (currently 2.2.0) — for parameter names and tags.
...
([`00-api-surface.md`](00-api-surface.md)) and the 2.2.0 Javadoc before
relying on a specific signature, per the lookup priority above.

This "New in 1.7.0" list is historical and is NOT the complete 2.2.0
surface — every release from 1.8 through 2.2 added further public API
(e.g. `addTableOfContents(...)`, `addPageReference(...)`, the
`toImage(...)` / `toImages(...)` output methods). The allow-list is the
authoritative, exhaustive set for the resolved target version; treat it,
not this section, as the completeness source.

## What moved in 2.0 — read this before porting 1.x code

The 2.x line reorganised the template surface. Anything below that a 1.x
skill or example still refers to is gone from this version:

| 1.x | 2.x |
|---|---|
| `com.demcha.compose.document.theme` | `com.demcha.compose.document.templates.core.theme` (`BrandTheme`, with `boxedClassic()` / `modernProfessional()` / `centeredHeadline()` factories) |
| `templates.builtins.InvoiceTemplateV1` and friends | per-kind presets: `templates.invoice.presets`, `templates.cv.presets`, `templates.coverletter.presets`, `templates.proposal.presets` |
| `templates.data.cv` / `.coverletter` / `.common` | `templates.cv.data`, `templates.coverletter.data` (invoice, proposal and schedule data stayed under `templates.data.*`) |
| the legacy `GraphCompose.pdf(...)` / `PdfComposer` surface | removed in 2.0 — the canonical entry point is `GraphCompose.document(...)` |

The layered CV surface (`templates.cv.components`, `.widgets`,
`.presets`) is new in this pack and has no 1.x equivalent. As always the
allow-list decides: if a symbol from the left column is not in
`00-api-surface.md`, it does not exist here.

## Known limitations

This skill describes the 2.2.x semantics conceptually. It does not
...
[`verifiedAgainst: 2.2.0`](../../../docs/versioned-skills.md) Javadoc.
