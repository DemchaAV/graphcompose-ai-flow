...
- **A positioned badge inside a nested stack will not move off its
  ...
  the project resolves 2.2.0; do not compensate with a canvas-drawn
  badge. See
...
## Verified findings from 2.2.0 template rendering

These three failures were hit (and fixed) while rendering a real
two-page CV template against `io.github.demchaav:graph-compose:2.2.0`.
...
- **`Bundled font resource not found: /fonts/google/<family>/...ttf`** at
  ...
  `1.0.0` as of engine 2.2.0; the `graph-compose-bundle` aggregate pulls
