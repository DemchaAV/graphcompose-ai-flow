...
- **`--task` and `--tasks`: which way, not just what exists.** A surface says a
  symbol exists; it cannot say which of three ways is right, and that is where
  wrong-API choices come from — a skills list in two columns is a row with
  weights, and nothing in a signature says so. `--tasks` lists the intents the
  pack's `routing/tasks.json` answers; `--task <id>` returns the decision: the
  recommended route and why, the alternatives with what each costs, the named
  engine constraints, and the symbols to verify. A miss exits 3 with the near
  matches, so a caller can branch on it. The route's `docs` anchors are paths in
  the GraphCompose repository — the bundle ships knowledge, not prose — and the
  answer says so rather than sending a reader after a file this workspace has
  never had. A route with no `confirmedBy` comes back `confirmed: false`, so an
  unreviewed recommendation does not read like a reviewed one.
- **`--surface`, and the surface in every answer.** A bundle pack splits the API
  five ways, and which surface a type sits in decides whether a template may
  call it at all. `--surface <name>` restricts the search; every answer now
  carries `surface`, and `stability` when it is not stable, because "does it
  exist" and "is it something I should be calling yet" are the same question
...
- **`create-template` and `revise-template` did not load in Claude Code.**
  Both descriptions were unquoted YAML scalars containing `: "` — a nested
  mapping to a YAML parser, so the host dropped the two skills that do the
  work while `plugin-package.test` (which split lines, never parsed) stayed
  green. All four descriptions are quoted, and the test now parses the
  frontmatter the way a loader does and refuses what YAML refuses.
- **`new-revision` created an empty folder** while the CHANGELOG (v0.10.0)
  and `revision-seal.mjs` both said it carried the body forward. Every real
  run copied the eight source files by hand, incompletely, or stayed in
  `revision-001` and rendered there seven times — which is why the corpus
  shows 358 `render-and-diff` runs for 50 revisions and every loop bound
  counting 1. It now carries the parent's sources (template, data, overflow
  data, asset request and assets, analysis, plan) and leaves render and review
  artifacts behind; `--empty` restores the old behaviour.
- **`render-and-diff` formed the loop verdict before the review existed.** On
  every fresh pass the last step answered `REVISE` with "no
  visual-review.json" and no focus — an exit code the skill said to branch on,
  carrying nothing — and the agent then ran `iterate-status` again. Without a
  review the pass now reports `awaiting-review` and says what to do; the
  verdict is formed only when there is a review to form it from.
- **Parent comparisons were not exact.** `--against parent` ran pixelmatch at
  its default threshold (0.1), which forgives a shifted anti-aliased edge —
  right against a rasterised design, wrong for the `exact-diff` and
  `region-diff` gates that quote `AE == 0`. It runs at threshold 0 now.
- **Telemetry filed runs under the wrong project when terminals ran in
  parallel.** `run-metrics` read `CLAUDE_SESSION_ID`, which nothing sets, and
  fell through to the most recently touched session file: six of eight
  recorded runs named another project's session. It reads
  `CLAUDE_CODE_SESSION_ID` — what Claude Code actually exports — first.
- **`preflight` surfaced none of what the workspace had learned.** It read
  observations only from the install tree, never from the workspace where
  `observations record` writes them; fourteen records in one real workspace
  were invisible to every later run's preflight. Both stores are read, each
  record marked `shipped` or `learned here`.
- **`preflight` reported `unknown` when there was no build file at
  `--project-dir`** even though the workspace's `resolved-version.json` knew
  the version and the build file it came from. It falls back to that record
  and says so (`graphCompose.source: "workspace-record"`).
- **`preflight` probed only `magick`,** ignoring `MAGICK_BINARY` (which
  `import-reference` and `typography` honour) and `convert`, so the
  repository's own devcontainer reported ImageMagick absent. It tries all
  three, and when the tool is installed but not on PATH — the common Windows
  case — says where it is and what to set.
- **`preflight` recommended the pair of commands the loop then rejects.** Its
  `nextCommands` named `render.mjs` + `iterate-status`, which leaves a
  revision `iterate-status` calls `unmeasured-render`. It names
  `render-and-diff`.
- **`revise-template` told the agent to run `render.mjs` and then
  `magick compare` by hand;** the gate is `render-and-diff --against parent`,
  and always was. The skill, `AGENTS.md`, `CLAUDE.md`, `architecture.md`,
  `scope-routing.md`, `pipeline.json` and `plugin-installation.md` now describe
  the metric that actually runs, and what ImageMagick is actually needed for.
- **`CLAUDE.md`, the Cursor, Windsurf and Copilot rule files** still taught
  the removed eleven-agent chain, a `prompts/` directory that does not exist,
  and `examples/<project>/revisions/` as where work lands. All four are short
  pointers to `AGENTS.md` now.
- **`npm test` failed on any machine with a newer plugin installed or a
  workspace inside the checkout.** Two preflight tests read the real
  `~/.claude/plugins` cache; two workspace tests resolved from the checkout's
  own directory. All four are isolated. `/graphcompose-flow/` and `*.7z` are
  gitignored, because a workspace created inside the checkout is the user's.
