...
/**
 * Revision-006 of the Noir Corporate CV template.
 *
...
public final class GeneratedCvTemplate {
    ...
    private static final double TOP_SURFACE_HEIGHT = FULL_PAGE_HEIGHT * 0.245;
    private static final double BODY_SURFACE_HEIGHT =
            FULL_PAGE_HEIGHT - TOP_SURFACE_HEIGHT - 1.0;

    // Internal spacing.
    private static final double SIDEBAR_PAD_X = 18.0;
    private static final double SIDEBAR_PAD_TOP = 44.0;
    ...
    private static final double CV_TEXT_OVERLAY_TOP = -CV_DIAMETER * 0.64;
    private static final double CV_TEXT_OVERLAY_BOTTOM = 45.0;
    private static final double SIDEBAR_RULE_THICK = 0.7;
    private static final double SIDEBAR_BOTTOM_FILL = PAGE_CONTENT_HEIGHT * 0.25;
    ...
    public void compose(DocumentSession document, NoirCorporateCvSpec spec) {
        ...
        document.pageFlow(page -> page
                ...
                .spacing(0)
                .addLayerStack(stack -> renderPageLayers(stack, spec)));
    }

    private void renderPageLayers(LayerStackBuilder stack, NoirCorporateCvSpec spec) {
        stack.name("PageSurfaceStack");
        stack.layer(buildPageSurfaceLayer(), LayerAlign.TOP_LEFT);
        stack.layer(buildContentLayer(spec), LayerAlign.TOP_LEFT);
    }

    private SectionNode buildPageSurfaceLayer() {
        return new SectionBuilder()
                .name("PageSurfaceLayer")
                .spacing(0)
                .addSection("TopDarkSurface", section -> section
                        .fillColor(DARK)
                        .spacer(FULL_PAGE_WIDTH, TOP_SURFACE_HEIGHT))
                .addSection("BodyPaperSurface", section -> section
                        .fillColor(WHITE)
                        .spacer(FULL_PAGE_WIDTH, BODY_SURFACE_HEIGHT))
                .build();
    }

    private SectionNode buildContentLayer(NoirCorporateCvSpec spec) {
        return new SectionBuilder()
                .name("ContentLayer")
    ...
    private void renderIdentity(SectionBuilder section, NoirCorporateCvSpec spec) {
        ...
        section.addParagraph(p -> p
                ...
                .margin(new DocumentInsets(CV_TEXT_OVERLAY_TOP, 0, CV_TEXT_OVERLAY_BOTTOM, 0)));
    }
    ...
    private void renderMain(SectionBuilder section, NoirCorporateCvSpec spec) {
        section.spacing(0)
                ...
                .addSection("MainBody", inner -> inner
                        .padding(new DocumentInsets(30.0, 0, 0, 0))
                        .spacing(30)
    ...
    private void renderNameBar(SectionBuilder section, NoirCorporateCvSpec spec) {
        section.fillColor(DARK)
                .padding(new DocumentInsets(76.0, MAIN_PAD_X, 88.0, MAIN_PAD_X))
                .spacing(4)
                .addParagraph(p -> p
    ...
    private void experienceItem(SectionBuilder section, ExperienceEntry entry) {
        ...
        if (!entry.highlights().isEmpty()) {
            section.addList(list -> list
                    ...
                    .lineSpacing(1.08)
                    .margin(new DocumentInsets(0, 0, 13, 24)));
        }
    }
