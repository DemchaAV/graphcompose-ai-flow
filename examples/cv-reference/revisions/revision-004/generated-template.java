...
/**
 * Revision-004 of the CV reference template.
 *
 * <p>Improvements over revision-003:</p>
 * <ul>
 *   <li>Expertise badge is {@code mdi:check-circle-outline} at 38pt — visibly
 *       large and matches the thin-line circle-with-check from the reference
 *       (vs. the heavy decagram badge revision-003 used at 22pt).</li>
 *   <li>Social icons are {@code entypo-social:*-with-circle} (filled black
 *       circle with a white brand glyph inside) at 13pt — they match the
 *       circular badges in the reference.</li>
 *   <li>Awards and References render as real two-column grids inside the
 *       Main column, built from {@link com.demcha.compose.document.dsl.TableBuilder}
 *       with two auto-width columns and node cells holding a section per entry.
 *       This removes the whitespace-cascaded text fallback used in
 *       revision-002 and revision-003.</li>
 *   <li>Every icon's document-space point size comes from
 *       {@code assets-manifest.json}; the Java side reads it via
 *       {@link IconSpec}. No more hard-coded per-token constants.</li>
 * </ul>
 */
