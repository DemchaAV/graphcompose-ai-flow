...
driving the fixture's no-arg `RowBasicFixtureDocument` adapter.
...
| `output.pdf` | present (committed baseline) | `tools/preview-renderer` via `RowBasicFixtureDocument` |
| `output.png` | present (committed baseline) | `tools/preview-renderer` via `RowBasicFixtureDocument` |
...
- **Render** (`tools/preview-renderer`): `node scripts/validate-skills.mjs
  --update-baseline` instantiates the no-arg `RowBasicFixtureDocument`,
