# expected-output for shape-container-card
...
driving the fixture's no-arg `ShapeContainerCardFixtureDocument` adapter.
...
| `output.pdf` | present (committed baseline) | `tools/preview-renderer` via `ShapeContainerCardFixtureDocument` |
| `output.png` | present (committed baseline) | `tools/preview-renderer` via `ShapeContainerCardFixtureDocument` |
...
- **Render** (`tools/preview-renderer`): `node scripts/validate-skills.mjs
  --update-baseline` instantiates the no-arg `ShapeContainerCardFixtureDocument`,
