# expected-output for table-basic
...
driving the fixture's no-arg `TableBasicFixtureDocument` adapter.
...
| `output.pdf` | present (committed baseline) | `tools/preview-renderer` via `TableBasicFixtureDocument` |
| `output.png` | present (committed baseline) | `tools/preview-renderer` via `TableBasicFixtureDocument` |
...
- **Render** (`tools/preview-renderer`): `node scripts/validate-skills.mjs
  --update-baseline` instantiates the no-arg `TableBasicFixtureDocument`,
