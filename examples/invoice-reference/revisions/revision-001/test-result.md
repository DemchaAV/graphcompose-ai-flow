...
| Layout snapshot is generated | PRESENT (illustrative) | `layout-snapshot.json` is committed but its bounding boxes are computed from the textual reference description, not from a real engine run. The `notes` field at the top of the file makes that explicit. |
